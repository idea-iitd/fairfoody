...
double get_route_plan_total_delivery_time(unordered_map<string, double> &delivered_time, vector<event> &route_plan, double global_time){
	double final_delivery_time = 0;
	double extra_delivery_time = 0;
	for (int i = 0; i < int(route_plan.size()); i++){
		if (route_plan[i].type == 1){
			order event_order = route_plan[i].order_obj;
			if(delivered_time[event_order.order_id] > final_delivery_time){
				final_delivery_time = delivered_time[event_order.order_id];
			}
			
		}
	}
    
	return final_delivery_time-global_time;
}
...
bool check_route_plan_sla_constraint(unordered_map<string, double> &delivered_time, vector<event> &route_plan){
	if (use_sla_constraint == false)
		return true;
	for (int i = 0; i < int(route_plan.size()); i++){
		if (route_plan[i].type == 1){
			order event_order = route_plan[i].order_obj;
			if (delivered_time[event_order.order_id] > (event_order.order_time + event_order.SLA))
				return false;
		}
	}
	return true;
